...
<div class="main-content flex-grow-1 p-4">
    ...
    <table id="tabelRuang" class="table table-bordered">
        <thead>
            <tr>
...
$(document).ready(function() {
    ...
    $('#tabelRuang').DataTable({
        ...
        "info": true,  // Display table info (showing X of Y entries)
    });

    // Dropdown Logic
    $('#selectProdi').change(function() {
        const prodi = $(this).val().trim().toLowerCase();
        $('#tabelRuang tr').each(function(index) {
            // Cek apakah baris ini adalah header (index 0 adalah header tabel)
            if (index === 0) {
                $(this).show(); // Pastikan header tabel selalu ditampilkan
            } else {
    ...
    // SweetAlert2 confirmation for "Setujui Semua Ruang"
    $('form[action="{{ route('setujui.semua.ruang') }}"]').on('submit', function(e) {
